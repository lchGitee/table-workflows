...
- `implementation.artifacts` 覆盖执行所需的脚本、DSL、运行器和锁定依赖文件；使用任何非标准库、共享运行时或外部 DSL 运行器时必须包含 `DEPENDENCY_LOCK` 产物，记录精确版本和可用的完整性信息，并由正常运行入口校验；仅记录外部路径、符号链接或笼统版本范围不满足要求；
...
结构指纹有两个来源，必须使用完全相同的提取字段、规范化和确定性序列化规则：

1. 创建或升级版本时，从已确认样本实际提取规范化结构描述，将其保存为 `basis`，并计算 manifest 中的期望指纹；
2. 每次运行时，从本次实际输入和结果模板重新提取规范化结构描述，计算观察指纹，再与期望指纹或明确的兼容规则比较。

读取 `basis` 并重新计算其哈希只能校验任务模板自身是否完整，不能代替第 2 步。运行回执必须记录每个实际文件的观察指纹和比较结论。结构描述中每个未列入 `allowedVariance` 的字段都必须被运行器实际观察和比较；工具无法可靠读取的字段属于未支持结构，应在创建输出前停止并转入优化模式，不能静默忽略。

发布版本前必须真实执行两类结构用例：一类只改变允许变化的数据或记录数量，确认不会误报漂移；另一类只改变契约相关结构，确认会在输出生成前阻断。工作簿负例至少改变一个非表头特征，例如可见性、隐藏行列、合并区域、命名区域或绘图对象。报告保存变体、期望、退出状态和输出是否产生，不能只用静态代码检查代替。
