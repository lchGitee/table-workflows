...
def structure(path):
    ...
    return {'fileType':'xlsx', 'sheets':sheets, 'definedNames':[xml_value(v) for v in workbook.defined_names.values()],
            ...
            'fieldTypes':{'订单号':'nonempty text unique','仓库代码':'nonempty text','件数':'finite number','发货状态':'nonempty text','收款状态':'exact 已收款 for 待发货; otherwise ignored'}}
...
def run(request_path, validate_candidate=False):
    ...
    receipt={'workflowId':'monthly-shipped','workflowVersion':3,'startedAt':now(),'exitStatus':1,
    ...
    try:
        ...
        if validate_candidate:
            ...
            if manifest['version']!=2 or snapshot['version']!=3 or snapshot['status']!='NEEDS_REVIEW':
        ...
        if request.get('workflowId')!='monthly-shipped' or request.get('workflowVersion')!=3 or request.get('schemaVersion')!='0.1':
        ...
        for rowno,row in enumerate(sheet.iter_rows(min_row=2,values_only=True),2):
            ...
            # 已发货全部计入；只新增待发货且已收款的订单，其他状态不扩大。
            if not (shipped=='已发货' or (shipped=='待发货' and paid=='已收款')):
                excluded+=1
                continue
        ...
        receipt['checks']={'shipment-or-paid-pending':'PASS','unique-order':'PASS','balanced-total':'PASS',
        ...
        receipt.update({'exitStatus':0,'outputHash':sha(output),'inputRecords':len(seen),'includedRecords':matched,
                        'excludedRecords':excluded,'unmatchedRecords':0,'outputRecords':len(rows),'includedTotal':sum(r[1] for r in rows),'reopened':True})
